...
def main():    
    ...
    paths = []
    depths = []
    cave = [['.' for _ in range(400)] for _ in range(200)]
    for path in data:
        match = re.findall('([0-9]+),([0-9]+) *-*>*',path)
        paths.append([(int(entry[1]),int(entry[0]) - 488) for entry in match])
    for path in paths:
        for i in range(1,len(path)):
            cave = draw_rock(path[i - 1], path[i], cave)
            depths.append(path[i-1][0])
    deep = max(depths)
    for i in range(400):
        cave[deep + 2][i] = '#'
    cave[0][12] = '+'
